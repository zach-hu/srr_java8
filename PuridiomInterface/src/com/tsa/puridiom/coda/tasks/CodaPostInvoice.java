...
import com.coda.www.efinance.schemas.inputext.ProblemLine;
import com.coda.www.efinance.schemas.inputext.Problems;
...
public class CodaPostInvoice extends Task{
	...
	public Object  executeTask (Object object) throws Exception
	{
		...
		try
		{
			...
			try {
				...
				InvoiceHeader header = (InvoiceHeader) incomingRequest.get("header");
				if (header == null) header = (InvoiceHeader) incomingRequest.get("invoiceHeader");
...
//				hdr.setUserCode(userName) ;
				...
				if (lines.size() > 1 || excludedCount == 0) {
					PostRequestPostOptions postOptions = new PostRequestPostOptions() ;
					postOptions.setPostto("books") ;

	//				PostToIntrayRequestPostToBooksOptions postToBooksOptions = new PostToIntrayRequestPostToBooksOptions() ;
	//				postToBooksOptions.setLogerrorsonly(false) ;
	//				postToBooksOptions.setStoponerror(true) ;
	//				postRequest.setPostToBooksOptions(postOptions) ;
	//				postRequest.setPostToBooksOptions(postToBooksOptions) ;

					txn.setHeader(hdr) ;
					txn.setLines(lnArray) ;

					postRequest.setPostData(postData) ;
					postRequest.setTransaction(txn) ;
					postRequest.setPostOptions(postOptions) ;
					...
					if (! HiltonUtility.isEmpty(proxyAddress)) {
						...
						parameters.put("serviceMethod", "post") ;
						parameters.put("serviceUrl", portAddress) ;
						parameters.put("paramObject",postRequest) ;
						parameters.put("userName", userName) ;
						parameters.put("userPassword", BlackBox.getDecrypt(passWord)) ;
						postResponse = proxy.wsProxyCall(proxyAddress, parameters) ;
					} else {
						postResponse = port.post(postRequest) ;
					}

					if (postResponse instanceof String) {
						System.out.println(postResponse) ;
					} else if (postResponse instanceof PostResponse) {
						Problems prob = ((PostResponse) postResponse).getProblems() ;
						ProblemLine[] probline  = prob.getLines();
						for (int x1= 0; x1 < probline.length; x1++) {
							System.out.println(probline[x1].getTypeDesc()) ;
						}
					}
				}
