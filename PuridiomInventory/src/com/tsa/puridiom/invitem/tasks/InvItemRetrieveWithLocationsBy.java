...
public class InvItemRetrieveWithLocationsBy extends Task
{
	public Object  executeTask (Object object) throws Exception
	{
		Map incomingRequest = (Map)object;
		DBSession dbs = (DBSession)incomingRequest.get("dbsession") ;
		StringBuffer queryString = new StringBuffer("from InvItem as invitem, InvLocation as invlocation where 1=1 ");
		queryString.append(" AND invitem.itemNumber = invlocation.id.itemNumber ");
		...
		if(incomingRequest.containsKey("InvItem_description"))
		{
			...
			queryString.append(" AND invitem.description = '" + description + "'");
		}
		...
		if (resultList != null && resultList.size() > 0)
		{
			for (int i=0; i < resultList.size(); i++) {
				Object resultObj[] = (Object[]) resultList.get(i);
				//add InvItem entity to invItemList to return as result
				itemList.add(resultObj[0]);
				//add InvLocation entity to invItemList to set in incomingRequest
				locationList.add(resultObj[1]);
			}
			//set InvLocationList in incomingRequest
			incomingRequest.put("invLocationList", locationList);
			//return InvItemList
			result = itemList;
		}
