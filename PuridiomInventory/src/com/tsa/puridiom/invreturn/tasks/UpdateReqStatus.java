 ...
 * Window - Preferences - Java - Code Style - Code Templates
 */
public class UpdateReqStatus extends Task
{
    ...
    public Object executeTask(Object object) throws Exception
    {
        try
        {
            ...
            String selectSql = "SELECT MIN( reqLine.status ) " +
            							...
            							"WHERE reqLine.icReqHeader = " + invReturn.getIcReqHeader().toString() ;
            ...
            {
                String reqStatus = DocumentStatus.INV_RETURNING;
                String minLineStatus = (String)result.get(0);
                if(rejectReturn.equalsIgnoreCase("Y"))
                {
                	reqStatus = DocumentStatus.REQ_REJECTED;
                }
                else
                {
	                if(minLineStatus.equals(DocumentStatus.INV_RETURNING))
	                {
	                    reqStatus = DocumentStatus.INV_RETURNING;
	                }
	                else if(minLineStatus.equals(DocumentStatus.INV_RETURNED))
	                {
	                    reqStatus = DocumentStatus.INV_RETURNED;
	                }
                }                
                
                String updateSelect = "UPDATE requisition_header SET status = ? WHERE ic_req_header = ?";
                Object [] args = {reqStatus, invReturn.getIcReqHeader().toString()};
