...
public class ApprovalSetData extends Task
{
	public Object executeTask(Object object) throws Exception
	{
        ...
        List	lineItemList = (List)incomingRequest.get("invoiceLineList") ;
        BigDecimal	icHeader = invoiceHeader.getIcPoHeader();
        BigDecimal	total = invoiceHeader.getInvoiceTotal();
		...
		try
		{
	        ...
	        for (int ix = 0;ix < appUdfSection.length ; ix++) {
	            ...
	            if (section.equalsIgnoreCase("header") ) {
	            	// Header
	        		Class cls = invoiceHeader.getClass() ;
					...
					value = (String) mth.invoke(invoiceHeader,null);
	...
	//        dataList.add(hRowList) ;
			...
			{
				InvoiceLine invoiceLine = (InvoiceLine) lineItemList.get(i);
		        ...
		        for (int ix = 0;ix < appUdfSection.length ; ix++) {
		            ...
		            if (appUdfSection[ix].equalsIgnoreCase("line")) {
		        		Class cls = invoiceLine.getClass() ;
		            	Method mth = cls.getMethod("get" + appUdfColumn[ix],null);
		            	Hashtable column = new Hashtable() ;
		            	column.put("section",appUdfSection[ix]) ;
		            	column.put("rulecolumn","fld" + (ix +1));
		            	...
		            	column.put("name",appUdfColumn[ix]);
						value = (String) mth.invoke(invoiceLine,null);
		            	column.put("value",Utility.ckNull(value).trim()) ;
		            	column.put("type",appUdfType[ix]);
		            	column.put("icheader",invoiceLine.getIcIvcHeader());
		            	column.put("icline",invoiceLine.getIcIvcLine()) ;
//		            	column.put("icaccount",invoiceLine.getIcAccount()) ;
		            	column.put("icaccount", new BigDecimal(0)) ;
		            	column.put("label",appUdfName[ix]);
		            	column.put("amount", invoiceLine.getLineTotal());
