...
public class InvoiceTaxAccountRecalculate extends Task	{
...

	{
		...
		try {
			List accountList = (List) incomingRequest.get("taxAccountList") ;
			if (accountList != null) {
				if (accountList.size() > 0) {
					...
					while (it.hasNext())
					{
						...
						if (allocMethod.substring(0, 1).equals("P"))
						{
								...
								ac.setAllocAmount(percent.multiply(bdTax).setScale(2,BigDecimal.ROUND_HALF_UP)) ;
								if (it.hasNext())
								{
						...
						if (! it.hasNext()) {
						    ac.setAllocAmount(bdTax.subtract(bdTotal));
						}
