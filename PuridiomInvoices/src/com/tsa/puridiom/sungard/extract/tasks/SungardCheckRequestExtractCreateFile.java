...
public class SungardCheckRequestExtractCreateFile extends Task {
	public Object  executeTask (Object object) throws Exception
	{
		...
		incomingRequest.put("RequisitionHeader_lastExtract", extractTimestamp);

		 if (Utility.isEmpty(outDir)) {
     		throw new Exception("The Check Request Extract Directory was not specified.");
     	}

         File dir = new File(outDir);
         if (!dir.isDirectory()) {
             Log.error(this, "The Extract Directory [" + dir + "] is not a valid directory.");
             throw new IOException("The Check Request Extract Directory is not a valid directory.");
         }
         if (!dir.canWrite()) {
             Log.error(this, "The Extract Directory [" + dir + "] does not have write access.");
             throw new IOException("The batch file cannot be created because the application does not have write access for the Check Request Extract Directory.");
         }
