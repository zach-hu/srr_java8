...
{
	public static String dateWhereClauseDecoder(String sqlWhere, List dateArguments, String organizationId, String userTimeZone, String userDateFormat) throws Exception
	{
			...
			for (int i = 0; i < conditions.length; i++)
			{
				...
				if (workString.matches(REGEX_ANY_CHARACTER + REGEX_COMPARATOR_OPERATOR + REGEX_WHITE_SPACES + ":" + REGEX_ANY_CHARACTER))
				{
					...
					if (value.startsWith(BrowseDateRanges.AS_DATE))	{
                        dateArguments.add(Dates.getDate(value.substring(BrowseDateRanges.AS_DATE.length()), userDateFormat));
						if(operator.equals("=")){
							Date limitDate = Dates.getDate(value.substring(BrowseDateRanges.AS_DATE.length()), userDateFormat);
							dateArguments.add(Dates.add(limitDate, 1));
							value = " = " + value;
							sqlWhere = sqlWhere.replaceFirst(value, " >= ? and " + property +" < ? ");
						}
						else{
					...
					} else if (value.indexOf(BrowseDateRanges.AS_FISCALSTART) > -1)
					{
						...
						dateArguments.add(new Date(Dates.getFiscalYearStartDate(Integer.parseInt(fyBegin), userTimeZone).getTime()));
					...
					{
						sqlWhere = sqlWhere.replaceFirst(value, "?");
						dateArguments.add(new Date(Calendar.getInstance().getTime().getTime()));
					...
					if (rangeDateType != 'N')
					{
						sqlWhere = sqlWhere.replaceFirst(operator + REGEX_WHITE_SPACES + value, ReportDates.getDateRangeExpression(operator, value, rangeDateType, property, dateArguments));
					}
				}
			}
		}
	...
	private static String getDateRangeExpression(String operator, String value, char rangeType, String property, List dateArguments)
	{
		String expression = "";
		Calendar calendar = Calendar.getInstance();
