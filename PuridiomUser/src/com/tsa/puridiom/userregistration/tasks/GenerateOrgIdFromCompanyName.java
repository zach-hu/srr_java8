...
public class GenerateOrgIdFromCompanyName extends Task {
	public Object executeTask(Object object) throws Exception {
		Map incomingRequest = (Map) object;
		Object result = null;

		try {
			String name = (String) incomingRequest.get("companyName");
			String scode = "";
			String fword, sword;
			String test;

			if (Utility.isEmpty(name)) {
				throw new RegistrationException("Company Name cannot be empty.");
			}
						...
						switch (fword.length()) {
							...
							default :
								if (sword.length() == 1) {
									...
									scode = fword.substring(0, 3) + sword.substring(0, sword.length()) + name.substring(0, 1);
								}
						}
					} else {
						switch (fword.length()) {
			...
			scode = scode + Dates.today("MMyyssSSS");
