...
public class ApproveRServlet extends HttpServlet
{
	...
	protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		try
		{
			...
			message = this.approveDocument(approvalLink, approvalLinkData[1]);
	...
	private String approveDocument(ApprovalLink approvalLink, String organizationId) throws Exception
	{
		String message = "";
		PuridiomProcessLoader processLoader = new PuridiomProcessLoader(organizationId);
		PuridiomProcess process = processLoader.loadProcess("requisition-approve-by-text-email.xml");
		Map incomingRequest = new HashMap();
		...
		for (Iterator iter = approvalLogList.iterator(); iter.hasNext();)
		{
			...
			if (alUser.equals(akUser) && approved.equals("Y"))
			{
				message = "Your approval for Requisition #" + requisitionHeader.getRequisitionNumber() + ", has completed!";
			}
		}

		if (HiltonUtility.isEmpty(message))
		{
			message = "Requisition #" + requisitionHeader.getRequisitionNumber() + " has already been processed and is no longer in your queue. ";
		}

		Log.debug(this, "Message for approve: ***" + message + "***");
