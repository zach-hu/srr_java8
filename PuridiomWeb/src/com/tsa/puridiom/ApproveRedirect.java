 ...
 */
public class ApproveRedirect extends HttpServlet
{
    ...
    public void approveMe(HttpServletRequest request, HttpServletResponse response) throws ServletException
    {
    	long startRequest = System.currentTimeMillis();
        Log.debug(this, request.getParameter("userId") + " - HiltonServletController.doPost BEGIN");
        try
        {
        	String toPage = "/approval/mail_approve.jsp";
            String link = request.getRequestURI();
            request.setAttribute("requestURI", link);
            int lastSlash = link.lastIndexOf("ApproveRedirect/");
            if(lastSlash > 0)
            {
