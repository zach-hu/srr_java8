...
public class ResetSupplierPasswordRedirectServlet extends HttpServlet
{
    ...
    public void resetPasswordRedirect(HttpServletRequest request, HttpServletResponse response) throws ServletException
    {
        ...
        try
        {
        	String toPage = "/supplierportal/user/chg_pswd_redirect.jsp";
            String link = request.getRequestURI();
            System.out.println("LInk is " + link);
            request.setAttribute("requestURI", link);
            int lastSlash = link.lastIndexOf("ResetSupplierPasswordRedirectServlet/");
            if(lastSlash > 0)
            {
