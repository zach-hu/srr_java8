...
<%
	...
	String poAction = (String)request.getAttribute("reqaction");
	...
	if (poAction == null) { poAction = "FORWARD"; }
...
<input type=hidden name="PoHeader_icPoHeader" value="<%=poHeader.getIcPoHeader()%>">
<input type=hidden name="PoLine_icPoHeader" value="<%=poHeader.getIcPoHeader()%>">
<input type=hidden name="PoHeader_poNumber" value="<%=poHeader.getPoNumber()%>">
<input type=hidden name="PoHeader_releaseNumber" value="<%=poHeader.getReleaseNumber()%>">
<input type=hidden name="PoHeader_revisionNumber" value="<%=poHeader.getRevisionNumber()%>">
<input type=hidden name="PoHeader_fiscalYear" value="<%=poHeader.getFiscalYear()%>">
<input type=hidden name="PoHeader_displayNumber" value="<%=poHeader.getDisplayPoNumber()%>">
<input type=hidden name="PoHeader_buyerCode" value="<%=poHeader.getBuyerCode()%>">
<input type=hidden name="poaction" value="">
		...
		<tr>
			<td valign="top" width="50px" height="30px">
				...
				<tr>
					...
					<div style="margin-left:10px; margin-right:10px" class="hdr12"><%=DictionaryManager.getLabelsInstance(oid, language).getLabel(oid, "po-generateRoutingList", "Generate Routing List")%></div>
					</td>
				</tr>
				</table>
			</td>
			...
			<td valign="bottom" align="right" height="30px" width="100%">
				...
				<tr>
				  <td align=right><b><%=DictionaryManager.getLabelsInstance(oid, language).getLabel(oid, "poNumber", "Order #")%>:</b></td>
				  <td width=125px><%=s_po_number%></td>
				</tr>
				<tr>
				  <td align=right><b><%=DictionaryManager.getLabelsInstance(oid, language).getLabel(oid, "status", "Status")%>:</b></td>
				  <td width=125px><%=DocumentStatus.toString(poHeader.getStatus())%></td>
				</tr>
				</table>
				<table cellpadding="0" cellspacing="0" border=0 width=100%>
				<tr>
...
<%		}%>
		...
		<tr><td><br></td></tr>
		<tr>
			<td class=label align=right><%=DictionaryManager.getLabelsInstance(oid, language).getLabel(oid, "poTotal", "Order Total")%>:</td>
			<td><%=HiltonUtility.getCurrency(poHeader.getTotal(), s_curr_code, oid)%></td><td>&nbsp</td>
			<td>[ <%=HiltonUtility.getCurrencyConvert(poHeader.getTotal(), s_curr_code, oid)%> ]</td>
		</tr>
		</table>
	</td>
</tr>
...
%>
		...
		<tr height="20px">
			...
			<% if (poAction.equalsIgnoreCase("FORWARD") && bPassed && routingList.size() > 1) { %>
				<tr height="25px">
		...
		<%	boolean authorityFound = false;
				...
				for (int i = 0; i < routingList.size(); i++) {
					...
					if (poAction.equalsIgnoreCase("FORWARD")) {
		...
		<%		if ((appLog.getRuleType().equals("MAN")) && poAction.equalsIgnoreCase("FORWARD")) { %>
...
<%	if (bPassed && poAction.equalsIgnoreCase("FORWARD") && !byPassRoutingList) { %>
<tr>
	<td align=center>
		<table border="0" cellspacing="0" cellpadding="0">
		<tr>
...
<%	if(!HiltonUtility.isEmpty(autoaward) && autoaward.equalsIgnoreCase("Y") && auto_award_rawa == 99)
	{ %>
		doSubmit('orders/po_forward_options.jsp', 'PoAward');
<%	}
	else if (byPassRoutingList && poAction.equals("FORWARD")) {%>
...
<%}%>
	...
	function doSubmitHide() {
		hideAreaWithBlock('_link');
		doSubmit('orders/po_review.jsp', 'DoNothing;PoRetrieve');
	}
...
<%	if (oid.equalsIgnoreCase("vse06p")) {%>
			popupParameters=popupParameters + ";AppRule_udf1Code=<%=poHeader.getDepartmentCode()%>";
<%	}%>
		} else {
			...
			for (var ia=0; ia < approvers; ia++) {
				...
				if (rule == approverUdfValues) {
					...
					if (eval(approverAmt) >= eval(amt)) {
						if (getApproverUserId(ia) == frm.PoHeader_buyerCode.value && !allowSelfApproval) {
						} else {
							authorityMet = true;
							break;
						}
