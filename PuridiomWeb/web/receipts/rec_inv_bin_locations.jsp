...
<%
	...
	if (invItem != null && !invItem.getUnitOfIssue().equals(poLineUom)) {
		displayFactor = true;
...
<% String	p_return_page = "";
	if(s_current_process.equalsIgnoreCase("STEP0")){
		p_return_page = "/receipts/rec_items_to_step_0.jsp";
	} else if(s_current_process.equalsIgnoreCase("STEP1")){
		p_return_page = "/receipts/rec_items_to_step_1.jsp";
	} else if(s_current_process.equalsIgnoreCase("STEP2")){
		p_return_page = "/receipts/rec_items_to_step_2.jsp";
	} else if(s_current_process.equalsIgnoreCase("STEP3")){
		p_return_page = "/receipts/rec_items_to_step_3.jsp";
	}
%>
...
<input type=hidden name="returnPage" value="<%=p_return_page%>">
...
<tr>
	...
	<td valign=bottom align=right>
		<b>Quantity Received:</b> <%=qtyReceived%>&nbsp;
...
<%	if (displayFactor) {%>
		<table border=0 cellpadding=0 cellspacing=0 width=775px>
		<tr><td class=error align=center>The Unit of Order specified on the receipt [<%=poLineUom%>] is different than the Unit of Issue specified for this inventory item [<%=invItemUoi%>].<br>Please provide a conversion factor.</td></tr>
		<tr>
			<td align=right>
						...
						<tr>
							<td align=center>
								...
								<tr>
									...
									<td <%=HtmlWriter.isVisible(oid, "rec-quantityUom")%> align=left width=6%><%=poLineUom%><tsa:hidden name="PoLine_UmCode" value="<%=poLineUom%>"/></td>
...
<!-- Hide script
	...
	var qtyReceived = <%=HiltonUtility.getFormattedQuantity(qtyReceived, oid)%>;
	var duomQtyReceived = <%=HiltonUtility.getFormattedQuantity(duomQtyReceived, oid)%>;

	var poUmCode = "<%=poLineUom%>" ;
...
<%	}%>
	...
	function addBinLocation() {
		...
		binLocationTableHTML = "<input type=hidden name=\"InvBinLocation_icRecLine\" value=\"<%=icRecLine%>\">" +
				...
				"<input type=hidden name=\"InvBinLocation_itemNumber\" value=\"<%=invItemBinNumber%>\">" +
...
//		myCell.innerHTML = "<table border=0 cellpadding=1 cellspacing=0 width=100% id=binLocation" + rowCount + "><input type=hidden id=\"InvBinLocation_tempIc\" value=\"<%=tempIc%>\"></table>";
		...
		myCell.width = "10%";
		myCell.align = "right";
		myCell.innerHTML = "<%=invItemBinCost%><input type=hidden name=\"InvBinLocation_cost\" value=\"<%=invItemBinCost%>\" size=9 style=\"text-align: right\">";
		...
		myCell.width = "6%";
		myCell.align = "left";
		myCell.innerHTML = "<input type=text name=\"PoLine_umCode\" value=\"<%=poLineUom%>\" size=11 onchange='upperCase(this); void(0);' disabled>";
		...
		myCell = myRow.insertCell();
		myCell.width = "12%";
		var reqnumber = '<%=headerEncoder.encodeForJavaScript(s_reqNumber)%>';
		myCell.innerHTML = "<input type=text id='reqnumberset' name=\"InvBinLocation_udf3Code\" size=12>";
		$('#reqnumberset').val(s_reqNumber);
		<%=HtmlWriter.cellVisibility(oid, "rec-bin-locbrw-inv-BN03")%>
		<%=HtmlWriter.cellDisplay(oid, "rec-bin-locbrw-inv-BN03")%>
	...
	<%	} %>
		...
		if (qtyAllocated != qtyReceived) {
			alert("You must allocated the quantity you are receiving [<%=qtyReceived%>].");
			return false;
		}
