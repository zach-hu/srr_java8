...
<%
	...
	String icReqHeader = (String) request.getAttribute("RequisitionLine_icReqHeader");
	String s_req_type = (String) request.getAttribute("requisitionType");

	if (s_req_type ==null) s_req_type = (String) request.getAttribute("RequisitionHeader_requisitionType") ;
	...
	if (reqLineList != null && reqLineList.size() > 0)
	{
		...
		icReqHeader = reqLine.getIcReqHeader().toString();
			...
			<% if (s_req_type.equalsIgnoreCase("M")) {%>
				<tsa:tr>
					<tsa:td noWrap="nowrap" cssClass="hdr12" valign="middle">
						<div style="margin-left:10px; margin-right:10px" class=hdr12><%=RequisitionType.toString(s_req_type, oid)%>&nbsp;History</div>
					</tsa:td>
				</tsa:tr>
			<% } else { %>
				<tsa:tr>
					<tsa:td noWrap="nowrap" cssClass="hdr12" valign="middle">
						<div style="margin-left:10px; margin-right:10px" class=hdr12><tsa:label labelName="requisitionHistory" defaultString="Requisition History"/></div>
					</tsa:td>
				</tsa:tr>
			<% } %>
				</table>
...
<tsa:tr>
	<tsa:td align="center">
		<span class=formType><tsa:label labelName="req-requisition" defaultString="Requisition"/> </span><span class="hdr12">#<%=headerEncoder.encodeForHTML(s_req_number)%></span>
	</tsa:td>
</tsa:tr>
<tsa:tr><tsa:td><br></tsa:td></tsa:tr>
<tsa:tr>
				...
				<tsa:td width="100%" align="center" valign="top">
					<table border="0" cellspacing="0" cellpadding="2" width="521px">
						<tsa:tr cssClass="mnav">
							<tsa:td field="req-lineNumber" noWrap="nowrap" width="35px" cssClass="mnav" align="right"><tsa:label labelName="req-hdg-lineNumber" defaultString="Line #"/>&nbsp;&nbsp;</tsa:td>
							<tsa:td field="req-itemNumber" noWrap="nowrap" width="175px" cssClass="mnav"><tsa:label labelName="req-hdg-itemNumber" defaultString="Item/Part #"/></tsa:td>
							<tsa:td field="req-quantity" noWrap="nowrap" width="70px" cssClass="mnav" align="right"><tsa:label labelName="req-hdg-quantity" defaultString="Quantity"/></tsa:td>
							<tsa:td width="15px" cssClass="mnav">&nbsp;</tsa:td>
							<tsa:td field="req-uom" noWrap="nowrap" width="70px" cssClass="mnav"><tsa:label labelName="req-hdg-uom" defaultString="UOM"/></tsa:td>
							<tsa:td field="req-unitCost" noWrap="nowrap" width="70px" cssClass="mnav" align="right"><tsa:label labelName="req-hdg-unitCost" defaultString="Unit Cost"/></tsa:td>
							<tsa:td field="req-extendedCost" noWrap="nowrap" width="70px" cssClass="mnav" align="right"><tsa:label labelName="req-hdg-extendedCost" defaultString="Ext Cost"/>&nbsp;</tsa:td>
			...
			<%	if (reqLineList == null)
					{
						...
						<tsa:tr>
							<tsa:td field="req-lineNumber" width="35px" align="right" valign="top"><%=HiltonUtility.getBigDecimalFormatted(reqLine.getLineNumber(), 0)%>&nbsp;</tsa:td>
							<tsa:td field="req-itemNumber" noWrap="nowrap" width="175px" valign="top"><b><%=reqLine.getItemNumber()%></b></tsa:td>
							<tsa:td field="req-quantity" width="70px" valign="top" align="right"><%=bd_quantity%></tsa:td>
							<tsa:td width="15px" >&nbsp;</tsa:td>
							<tsa:td field="req-uom" width="70px" valign="top"><%=uom%></tsa:td>
							<tsa:td field="req-unitCost" width="70px" valign="top" align="right"><%=bd_unit_price%></tsa:td>
							<tsa:td field="req-extendedCost" width="70px" valign="top" align="right"><%=bd_total%></tsa:td>
							<tsa:td align="center">&nbsp;</tsa:td>
						</tsa:tr>
						<tsa:tr>
							<tsa:td>&nbsp;</tsa:td>
							<tsa:td colspan="6"><%=reqLine.getDescription()%></tsa:td>
						</tsa:tr>
					<%}
					else
					{
						...
						<tsa:tr>
							...
							<tsa:td field="req-unitCost" width="70px" valign="top" align="right"><%=bd_unit_price%></tsa:td>
							<tsa:td field="req-extendedCost" width="70px" valign="top" align="right"><%=bd_total%></tsa:td>
							<tsa:td align="center">&nbsp;<a href="javascript: viewItemHistory('<%=reqLine.getIcLineHistory()%>', '<%=reqLine.getIcReqLine()%>'); void(0);"><img src="<%=contextPath%>/images/icon_history.gif" width="16px" height="16px" alt="Item History" border="0"/></a></tsa:td>
						</tsa:tr>
						<tsa:tr>
							<tsa:td>&nbsp;</tsa:td>
							<tsa:td colspan="6"><%=reqLine.getDescription()%></tsa:td>
...
<tsa:hidden name="requisitionNumber" value="<%=s_req_number%>"/>
<tsa:hidden name="RequisitionLine_icReqHeader" value="<%=icReqHeader %>"/>
...
<!-- Hide script
	...
	function viewItemHistory(icLineHistory, icReqLine)
	{
		...
		doSubmit('/requests/req_history.jsp', 'HistoryLogRetrieveByHistoryLine');
	}

	 function viewHistory()
	  {
	    doSubmit('/requests/req_history.jsp', 'HistoryLogRetrieveByHistoryHeader');
	  }
