...
<tr>
	...
	<td valign=bottom align=right border=0 width=*>
		<div id="filterTextDisplay"></div>
		<table cellpadding="0" cellspacing="0" border=0 width=100%>
			<tr><td width=100% height=1px class=lightShadow><img src="<%=contextPath%>/supplierportal/images/none.gif" width="1" height="1" /></td></tr>
...
<%	for (int itc=0; itc < tabCount; itc++) {%>
					<td valign=top height=18px><img src="<%=contextPath%>/supplierportal/images/none.gif" border=0 width=11px height=1px></td>
<%	}%>
					<td valign=top height=18px><a href="javascript: <%=jsFunction%>('<%=code%>'); void(0);" id="link<%=code%>" onclick="setCurrentPosition();"><img src="<%=contextPath%>/supplierportal/images/<%=img%>" border=0 class="<%=imgClass%>" <% if (img.equals("none.gif")) {%>height=1px width=11px<%}%>></a></td>
					<td height=18px class=browseRow width=100% valign=top>
<%				if (allowBrowse.equalsIgnoreCase("true") && isLastLevelCodeForSelect) {%>
