		...
		if (b_low_subtotal && (b_bids_entered || !b_require_entry )) {
			if (b_show_low_amt) {
				...
				if (bd_low_bid.compareTo(new BigDecimal("0.00000")) == 1) {
					s_msg = "Highest Subtotal Bid: " + bd_low_bid.toString();
				}
			}
			else if (rfqHeader.getLowestVendorId().equalsIgnoreCase(user.getVendorId())) {
				s_msg = "You are currently the highest bidder for this item!";
			}
			else {
				s_msg = "You are currently NOT the highest bidder for this item!";
			}
		}
...
%>
					...
					<tr>
						<td class=browseHdrDk colspan=2>
							...
							<tr>
								<td class=browseHdrDk>&nbsp;ITEM # <%=rfqLine.getItemNumber()%></td>
							</tr>
							</table>
						</td>
					</tr>
					<tr class=itemSummary>
						<td width=65% valign=top class=itemSummary>
							...
							<tr>
								<td class=itemSummary vAlign=top width=90px align=right><b>Description:</b></td>
							...
							<tr height=22px>
								<td class=itemSummary align=right><b>Manufacturer:</b></td>
								<td class=itemSummary><%=rfqLine.getMfgName()%>&nbsp;</td>
							</tr>
							<tr height=22px>
								<td class=itemSummary align=right><b>Model Number:</b></td>
								<td class=itemSummary><%=rfqLine.getModelNumber()%>&nbsp;</td>
							</tr>
...
<%		}%>
						...
						<td width=35% class=itemSummary valign=top>
							...
							<%@ include file="/supplierportal/sales/sale_bid_rows.jsp" %>
							</table>
						</td>
					</tr>
					</table>
