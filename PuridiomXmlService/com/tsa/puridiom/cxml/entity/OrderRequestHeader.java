...
public class OrderRequestHeader implements Serializable {
	 ...
	 */
	public static Element buildOrderRequestHeaderElement(
			OrderRequestHeader orderRequestHeaderObject) {
		Element orderRequestHeaderElement = new Element("OrderRequestHeader");

		orderRequestHeaderObject.setAttributes(orderRequestHeaderElement,
				orderRequestHeaderObject);

		orderRequestHeaderObject.setContent(orderRequestHeaderElement,
				orderRequestHeaderObject);

		return orderRequestHeaderElement;
	}

	private void setAttributes(Element orderRequestHeaderElement,
			OrderRequestHeader orderRequestHeaderObject) {
		/*
		...
		attributes.add(new Attribute("orderID", String
				.valueOf(orderRequestHeaderObject.getOrderID())));

		attributes.add(new Attribute("orderDate", String
				.valueOf(orderRequestHeaderObject.getOrderDate())));

		if (orderRequestHeaderObject.getOrderType() != null) {
			attributes.add(new Attribute("orderType", orderRequestHeaderObject
					.getOrderType()));
		}

		if (orderRequestHeaderObject.getType() != null) {
			attributes.add(new Attribute("type", orderRequestHeaderObject
					.getType()));
		}

		if (orderRequestHeaderObject.getOrderVersion() != null) {
			attributes.add(new Attribute("orderVersion",
					orderRequestHeaderObject.getOrderVersion()));
		}

		if (orderRequestHeaderObject.getIsInternalVersion() != null) {
			attributes.add(new Attribute("isInternalVersion",
					(orderRequestHeaderObject.getIsInternalVersion()
							.booleanValue()) ? "yes" : ""));
		}

		if (orderRequestHeaderObject.getAgreementID() != null) {
			attributes.add(new Attribute("agreementID",
					orderRequestHeaderObject.getAgreementID()));
		}

		if (orderRequestHeaderObject.getAgreementPayloadID() != null) {
			attributes.add(new Attribute("agreementPayloadID",
					orderRequestHeaderObject.getAgreementPayloadID()));
		}

		if (orderRequestHeaderObject.getRequisitionID() != null) {
			attributes.add(new Attribute("requisitionID",
					orderRequestHeaderObject.getRequisitionID()));
		}

		if (orderRequestHeaderObject.getShipComplete() != null) {
			attributes
					.add(new Attribute("shipComplete",
							(orderRequestHeaderObject.getShipComplete()
									.booleanValue()) ? "yes" : ""));
		}
	...
	private void setContent(Element orderRequestHeaderElement,
			OrderRequestHeader orderRequestHeaderObject) {
		/*
		...
		totalElement.addContent(Money
				.buildMoneyElement(orderRequestHeaderObject.getTotal()));
		...
		if (orderRequestHeaderObject.getShipTo() != null) {
			...
			shipToElement.addContent(AddressShipBillTo
					.buildAddressElement(orderRequestHeaderObject.getShipTo()));
		...
		billToElement.addContent(AddressShipBillTo
				.buildAddressElement(orderRequestHeaderObject.getBillTo()));

		orderRequestHeaderElement.addContent(billToElement);

		if (orderRequestHeaderObject.getShipping() != null) {
			orderRequestHeaderElement.addContent(Shipping
					.buildShippingElement(orderRequestHeaderObject
							.getShipping()));
		}

		if (orderRequestHeaderObject.getTax() != null) {
			orderRequestHeaderElement.addContent(Tax
					.buildTaxElement(orderRequestHeaderObject.getTax()));
		}

		// TODO Payment?, PaymentTerm*

		for (Iterator iter = orderRequestHeaderObject.getContact().iterator(); iter
				.hasNext();) {
			orderRequestHeaderElement.addContent(Contact
					.buildContactElement((Contact) iter.next()));
		}

		if (orderRequestHeaderObject.getComments() != null) {
			orderRequestHeaderElement.addContent(Comments
					.buildCommentsElement(orderRequestHeaderObject
							.getComments()));
		}

		// TODO Followup?, DocumentReference?, SupplierOrderInfo?, Extrinsic*
