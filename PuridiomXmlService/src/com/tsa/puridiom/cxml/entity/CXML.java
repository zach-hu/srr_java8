...
public class CXML implements Serializable
{
	...
	public static final String DO_NOT_RETRY_STATUS_CODE = "400";

	public static final String RETRY_STATUS_CODE = "500";
	...
	public String getFormatTimeStamp(CXML cXMLObject)
	{
		StringBuffer timeStamp = new StringBuffer();
		String timeZoneId = TimeZone.getDefault().getID();
		Calendar aGMTCalendar = Calendar.getInstance(TimeZone.getTimeZone(timeZoneId));
		if(cXMLObject.getTimestamp() != null)
		{
			timeStamp.append(new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss").format(cXMLObject.getTimestamp()));
	...
	private void setAttributes(Element cxmlDocument, CXML cXMLObject)
	{
		...
		if (cXMLObject.getTimestamp() != null)
		{
			timestamp = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss.SS").format(cXMLObject.getTimestamp());
		}

		attributes.add(new Attribute("version", cXMLObject.getVersion()));

		attributes.add(new Attribute("payloadID", cXMLObject.getPayloadID()));

		attributes.add(new Attribute("timestamp", cXMLObject.getFormatTimeStamp(cXMLObject)));

		if (cXMLObject.getSignatureVersion() != null)
		{
			attributes.add(new Attribute("signatureVersion", cXMLObject.getSignatureVersion()));
		}

		if (cXMLObject.getLang() != null)
		{
			attributes.add(new Attribute("lang", cXMLObject.getLang(), Namespace.getNamespace("xml", "http://www.w3.org/1999/xhtml")));
		}
	...
	private void setContent(Element cxmlDocument, CXML cXMLObject)
	{
		...
		{
			cxmlDocument.addContent(new Response().buildResponseElement(cXMLObject.getResponse()));
		} else
		{
			cxmlDocument.addContent(new Header().buildHeaderElement(cXMLObject.getHeader()));

			if (cXMLObject.getRequest() != null)
			{
				cxmlDocument.addContent(new Request().buildRequestElement(cXMLObject.getRequest()));
			} else
			{
				// TODO Message
			}
	...
	public static Document buildCXMLDocument(CXML cXMLObject)
	{
		...
		return new Document(cxmlDocument);
	}

	public static void toFileSystem(Document cxmlDocument, String organizationId, String fileName) throws Exception
	{
		FileOutputStream out = new FileOutputStream(DictionaryManager.getInstance("host", organizationId).getProperty("reportsOut", "") + "cxml" + File.separator + fileName + ".xml");
...
//		serializer.output(cxmlDocument, System.out);

		serializer.output(cxmlDocument, out);
		out.flush();
		out.close();
	}
	
	public static void toFileSystem(Document cxmlDocument, String omitEncoding, String organizationId, String fileName) throws Exception
	{
		FileOutputStream out = new FileOutputStream(DictionaryManager.getInstance("host", organizationId).getProperty("reportsOut", "") + "cxml" + File.separator + fileName + ".xml");

		XMLOutputter serializer = new XMLOutputter("", true);
		if (omitEncoding.equals("Y")) 
		{
			serializer.setOmitEncoding(true);
		} 
		else 
		{
			serializer.setOmitEncoding(false);
		}
...
//		serializer.output(cxmlDocument, System.out);
	 ...
	 */
	public static String toString(Document cxmlDocument, String omitEncoding)
	{
		...
		return xmlOutputter.outputString(cxmlDocument);
	}
	...
	public class Header implements Serializable
	{
		...
		private void setContent(Element headerElement, Header headerObject)
		{
			 ...
			 */
			headerElement.addContent(FromTo.buildFromToElement(Params.FROM_ELEMENT_NAME, headerObject.getFrom()));

			headerElement.addContent(FromTo.buildFromToElement(Params.TO_ELEMENT_NAME, headerObject.getTo()));

			headerElement.addContent(Sender.buildSenderElement(headerObject.getSender()));
	...
	public class Request implements Serializable
	{
		...
		private void setAttributes(Element requestElement, Request requestObject)
		{
			...
			{
				attributes.add(new Attribute("deploymentMode", requestObject.getDeploymentMode()));
			}

			if (requestObject.getId() != null)
			{
				attributes.add(new Attribute("Id", String.valueOf(requestObject.getId())));
			}
		...
		private void setContent(Element requestElement, Request requestObject)
		{
			...
			{
				requestElement.addContent(OrderRequest.buildOrderRequestElement(requestObject.getOrderRequest()));
			}
			// TODO else { .....
		}
	}
	...
	public class Response implements Serializable
	{
		...
		private void setAttributes(Element responseElement, Response responseObject)
		{
			/*
			 * Id ID #IMPLIED
			 */
			...
			{
				attributes.add(new Attribute("Id", String.valueOf(responseObject.getId())));
			}
		...
		private void setContent(Element responseElement, Response responseObject)
		{
			 ...
			 */
			responseElement.addContent(Status.buildStatusElement(responseObject.getStatus()));
