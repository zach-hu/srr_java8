...
public class OrganizationPackageRetrieveRequisitionerCount extends Task
{
	public Object  executeTask (Object object) throws Exception
	{
		...
		try {
			...
			StringBuffer queryString = new StringBuffer("select sum(organizationpackage.requisitionerCount) from OrganizationPackage organizationpackage where 1=1 ");
			...
			BigDecimal requisitionerCount = new BigDecimal(0);

			if (resultList != null && resultList.size() > 0)
			{
				requisitionerCount = (BigDecimal) resultList.get(0);
				if (requisitionerCount == null) {
					requisitionerCount = new BigDecimal(0) ;
				}
			}
			result = requisitionerCount;
