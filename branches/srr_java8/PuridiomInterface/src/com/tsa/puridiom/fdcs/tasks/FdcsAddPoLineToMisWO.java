...
import iseries.wsbeans.addpolinetomiscwo.*;
import iseries.wsbeans.addpolinetomiscwo.xsd.* ;
...
public class FdcsAddPoLineToMisWO extends Task{
		...
		try
		{
			...
			try {
				if (portAddress.length() > 0) {
					portAddress = portAddress + "/AddPOLineToMiscWO" ;
				}
				AddPOLineToMiscWOSOAP11BindingStub port = null ;
				if (proxyAddress.length() > 0) {
					AddPOLineToMiscWOLocator locator = new AddPOLineToMiscWOLocator() ;
					if (portAddress.length() > 0) {
						try {
							URL url = new URL(portAddress) ;
							port = (AddPOLineToMiscWOSOAP11BindingStub) locator.getAddPOLineToMiscWOSOAP11port_http(url) ;
						}
						catch (Exception E)
						{
					...
					if (port == null) {
						port = (AddPOLineToMiscWOSOAP11BindingStub)locator.getAddPOLineToMiscWOSOAP11port_http() ;
					}
				...
				POI950Input input = new POI950Input() ;
				input.set_PI_CHGCD(chargeCodeFiller + chargeCode) ; 	// charge code
				input.set_PI_EPIDNO(employeeNo) ;   // Employee Number
				...
				POI950Result res = null ;
				if (! HiltonUtility.isEmpty(proxyAddress)) {
					WSProxyCall proxy = new WSProxyCall() ;
					Map parameters = new HashMap() ;
					parameters.put("serviceName", "iseries.wsbeans.addpolinetomiscwo.AddPOLineToMiscWO") ;
					parameters.put("serviceLocatorMethod", "getAddPOLineToMiscWOSOAP11port_http") ;
					parameters.put("serviceMethod", "poi950") ;
					parameters.put("serviceUrl", portAddress) ;
					parameters.put("paramObject",input) ;
					parameters.put("userName", userName) ;
					parameters.put("userPassword", passWord) ;
					res = (POI950Result) proxy.wsProxyCall(proxyAddress, parameters) ;
				} else {
					res = port.poi950(input) ;
				}
				System.out.println("SQN01: " + res.get_PO_SQN01()) ;
				if (! HiltonUtility.isEmpty(res.get_PO_ERRORCODE())){
					errorStr = "AddPoLineToMiscWO - Order: " + poNumber.toString() + " Line: " + lineNumber.toString() + " Error: "+ res.get_PO_ERRORCODE() + " - " + res.get_PO_ERRORDESC() ;
