...
public class InvoiceOtherAccountRecalculate extends Task	{
...

	{
		...
		try {
			List accountList = (List) incomingRequest.get("otherAccountList") ;
			if (accountList != null) {
				if (accountList.size() > 0) {
					...
					while (it.hasNext())
					{
						...
						if (allocMethod.substring(0, 1).equals("P"))
						{
								...
								ac.setAllocAmount(percent.multiply(bdOther).setScale(2,BigDecimal.ROUND_HALF_UP)) ;
								if (it.hasNext())
								{
						...
						if (! it.hasNext()) {
						    ac.setAllocAmount(bdOther.subtract(bdTotal));
						}
