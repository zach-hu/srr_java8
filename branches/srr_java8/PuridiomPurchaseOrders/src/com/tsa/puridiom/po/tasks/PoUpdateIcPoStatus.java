...
public class PoUpdateIcPoStatus extends Task
{
	public Object executeTask(Object object) throws Exception
	{
		...
		try
		{
			...
			String sql = "Select poHeader.icPoHeader from PoHeader as poHeader where poHeader.poNumber = ? AND poHeader.releaseNumber = ?" +
			...
			String revision = (String)incomingRequest.get("lastRevision");

			if(!revision.equals("C"))
			{
				BigDecimal releaseNumber  = new BigDecimal(release);
				BigDecimal revisionNumber = new BigDecimal(revision.trim());
				...
				{
					BigDecimal ic = (BigDecimal)resultList.get(0);
					incomingRequest.put("originalIc", ic);
				}
			}
