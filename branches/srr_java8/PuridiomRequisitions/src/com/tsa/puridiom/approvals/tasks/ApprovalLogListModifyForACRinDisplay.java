...
public class ApprovalLogListModifyForACRinDisplay extends Task {
	...
	public Object executeTask(Object object) throws Exception	{
		...
		List newACRroutingList = new ArrayList();
        try {
        	...
        	if(incomingRequest.containsKey("acrPoolList") && incomingRequest.get("acrPoolList")!=null && ((List)incomingRequest.get("acrPoolList")).size()>=1)
        	{
        		...
        		for (int ir=0; ir < finalRuleList.size(); ir++) {
        			...
        			routingList = (List) ruleMap.get("ruleRoutingList");
        			...
        			if((acrPoolList!=null & acrPoolList.size()>0)&&(routingList!=null & routingList.size()>0)){
        				...
        				for(int c = 0 ; c < acrPoolList.size();c++ )
        				{
        					...
        					if(belongerToARC != null)
        					{
        						...
        						moveToLast(routingList,belongerToARC,newACRroutingList);
        					}
        		...
        		//moving ACR member to new RuleList	
        		if((List)incomingRequest.get("acrPoolList") != null){
        		String newApprovalRule  = ((AppPooluser)((List)incomingRequest.get("acrPoolList")).get(0)).getComp_id().getPoolid();
        		finalRuleList.add(newApprovalRule);
        		Map newRuleMap = new HashMap();
        		newRuleMap.put("amountToApprove", new BigDecimal(0));
        		newRuleMap.put("ruleRoutingList", newACRroutingList);
        		routingListGroups.put(newApprovalRule, newRuleMap);
	...
	private void moveToLast(List routingList,ApprovalLog arc, List newACRroutingList)
	{
		ApprovalLog res = null;
		for(int c = 0; c < routingList.size();c++)
		{
