...
<%
	...
	String contactType = HiltonUtility.ckNull((String) request.getAttribute("Contact_contactType"));
	...
	if (contactPK == null) {
		...
		contactPK.setContactType(contactType);
...
<%	} else {%><b><%=contactType%></b><%}%>
						<b><%=DictionaryManager.getLabelsInstance(oid, language).getLabel(oid, "contactSupplier", "Contact for Supplier")%> '<%=contactPK.getVendorId()%>'</b>					</td>
				</tr>
...
%>
<% if (( (String)request.getAttribute("pageFrom")).indexOf("supplier_insurance") >=0) {
	 if (role.getAccessRights("VEND") > 1) {
		if (oid.equalsIgnoreCase("MSG07P")) { %>
		<td width=50% align=center><div id="pxbutton"><a href="javascript: submitThis('/admin/supplier/supplier_insurance.jsp', '<%=updateHandler%>;VendorRetrieveById;VendorInsuranceDefaultRetrieveById;ContactInsuranceRetrieveByVendorId;VendorInsuranceDefaultInsCategoryLevelValidateVendor'); void(0);">Save</a></div></td>
		<% } else { %>
		<td width=50% align=center><div id="pxbutton"><a href="javascript: submitForm('/admin/supplier/supplier_insurance.jsp', '<%=updateHandler%>;VendorRetrieveById;VendorInsuranceDefaultRetrieveById;ContactInsuranceRetrieveByVendorId;VendorInsuranceDefaultInsCategoryLevelValidateVendor'); void(0);">Save</a></div></td>
<%		 }
	 }
%>
		<td width=50% align=center><div id="pxbutton"><a href="javascript: doSubmit('/admin/supplier/supplier_insurance.jsp', 'VendorRetrieveById;VendorInsuranceDefaultRetrieveById;ContactInsuranceRetrieveByVendorId;VendorInsuranceDefaultInsCategoryLevelValidateVendor'); void(0);">Return</a></div></td>
<% } else if (( (String)request.getAttribute("pageFrom")).indexOf("supplier_compliances") >=0) {
	 if (role.getAccessRights("VEND") > 1) {
		if (oid.equalsIgnoreCase("MSG07P")) { %>
		<td width=50% align=center><div id="pxbutton"><a href="javascript: submitThis('/admin/supplier/supplier_compliances.jsp', '<%=updateHandler%>;VendorInsuranceDefaultRetrieveById;ContractsRetrieveByVendorId;ContactComplianceRetrieveByVendorId'); void(0);">Save</a></div></td>
		<% } else { %>
		<td width=50% align=center><div id="pxbutton"><a href="javascript: submitForm('/admin/supplier/supplier_compliances.jsp', '<%=updateHandler%>;VendorInsuranceDefaultRetrieveById;ContractsRetrieveByVendorId;ContactComplianceRetrieveByVendorId'); void(0);">Save</a></div></td>
<% 		}
	 }
%>
		<td width=50% align=center><div id="pxbutton"><a href="javascript: doSubmit('/admin/supplier/supplier_compliances.jsp', 'VendorInsuranceDefaultRetrieveById;ContractsRetrieveByVendorId;ContactComplianceRetrieveByVendorId'); void(0);">Return</a></div></td>
<% } %>
...
<%	} %>
	...
	function validateForm() {
		...
		if (handlers.indexOf("<%=updateHandler%>") >= 0) {
			if (frm.contactPassword.value != "*****") {
				setHiddenFields("<input type=hidden name=\"Contact_contactPassword\" value=\""+frm.contactPassword.value+"\"></input>");
				<%--setHiddenFields("<tsa:hidden name=" + '"' + "Contact_contactPassword"+ '"' + " value=" + '"' + frm.contactPassword.value + '"' + "/>");--%>
			}
		}
		return true;
	}
	...
	//	doSubmit('/admin/supplier/supplier_contact_list.jsp', 'ContactUpdate;ContactAddressRetrieveBySupplier');
		doSubmit('/admin/supplier/supplier_contact_list.jsp', 'ContactSetAsDefault;ContactAddressRetrieveBySupplier');
	}
