	...
	<td valign=top width=135px height=30px>
		...
		<tr>
			<td nowrap class=hdr12 valign=middle>
				<div style="margin-left:10px; margin-right:10px" class=hdr12>Copies</div>
			</td>
		</tr>
		</table>
	</td>
			...
			<TR>
				...
				<TABLE WIDTH="100%" ALIGN="CENTER" BORDER="1" CELLSPACING="2" CELLPADDING="2" >
					<TR align="center" CLASS="hdr12">
						...
						<TD>Print Sides</TD>
						<TD>Print Copies</TD>
						<TD>Marginal Words</TD>
						<TD><a href="javascript: browseStd('InvFormPart_blockout','FBLK');">BlockOut</a></TD>
						<TD>Other</TD>
					</TR>
<%
	...
	int i_rowcount = 0;
		...
		if (invFormList != null)
		{System.out.println(invFormList.size());
...
%>
					<TR align="center" >
						...
						<TD>
							<select name="InvFormPart_printSides" <%=s_set_row%>>
								<option <% if (Utility.getBigDecimalFormatted(invFormPart.getPrintSides(),0).compareTo(new BigDecimal(1)) == 0) { %> SELECTED <% } %> value="1">One</OPTION>
								<option <% if (Utility.getBigDecimalFormatted(invFormPart.getPrintSides(),0).compareTo(new BigDecimal(2)) == 0) { %> SELECTED <% } %> value="2">Two</OPTION>
							</select>
						</TD>
						<TD ><input type="text" name="InvFormPart_printCopies" size=15 value = "<%=Utility.getBigDecimalFormatted(invFormPart.getPrintCopies(),0)%>" style="text-align:right" <%=s_set_row%>></TD>
						<TD ><input type="text" name="InvFormPart_marginWords" size=35 value = "<%=Utility.ckNull(invFormPart.getMarginWords())%>" <%=s_set_row%>></TD>
						<TD ><input type="text" name="InvFormPart_blockout" size=20 value = "<%=Utility.ckNull(invFormPart.getBlockout())%>" <%=s_set_row%>></TD>
						<TD ><input type="text" name="InvFormPart_copiesUdf" size=25 value = "<%=Utility.ckNull(invFormPart.getCopiesUdf())%>" <%=s_set_row%>></TD>
					</TR>
<% 	i_rowcount++;
	} }%>
				</TABLE>
...
<table border=0 cellpadding=0 cellspacing=0 width=680px>
<tr>
	<td width=50% align=center><a href="javascript: doSubmit('/inventory/forms/inv_item_copies.jsp', 'InvFormPartUpdateMultiple;InvFormPartRetrieve'); void(0);"><img class=button src="<%=contextPath%>/images/button_save.gif" border=0></a></td>
