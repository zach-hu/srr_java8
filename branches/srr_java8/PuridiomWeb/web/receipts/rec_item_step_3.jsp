...
<%
	...
	/*if((s_rec_status.compareTo(DocumentStatus.RCV_STEP_3) == 0) && uid.equals(receiptHeader.getAssignedUserStep3())){
		allowEdit = true;
	}*/
	...
	if (HiltonUtility.isEmpty(s_return_page)) {
		s_return_page = "/receipts/rec_items_to_step_3.jsp";
	}
...
<input type=hidden name="isEndStep" value="true">
...
<input type=hidden name="ReceiptLine_qtyStep2Accepted" value="<%=receiptLine.getQtyStep2Accepted()%>">
<% } else { %>
<input type=hidden name="ReceiptLine_qtyStep1Accepted" value="<%=receiptLine.getQtyStep1Accepted()%>">
<% } %>
		...
		<tr>
			<td width=100% align=center valign=top>
				...
				<tr>
					...
					<td <%=HtmlWriter.isVisible(oid, "rec-quantityToDeliver")%> width=7% class=browseHdr nowrap align=center><%=DictionaryManager.getLabelsInstance(oid, language).getLabel(oid, "rec-quantityToDeliver", "Deliver")%></td>
					...
					<td <%=HtmlWriter.isVisible(oid, "rec-quantityDelivered")%> width=14% class=browseHdr nowrap align=center><%=DictionaryManager.getLabelsInstance(oid, language).getLabel(oid, "rec-quantityDelivered", "Delivered")%></td>
...
%>
				<tr class=browseRow>
					...
					<td <%=HtmlWriter.isVisible(oid, "rec-quantityToDeleiver")%> width=7% class=browseRow nowrap align="center">
						<%=HiltonUtility.getFormattedQuantity(qtyToDeliver, oid)%> (<%=uom%>)</td>
					...
					<!-- Still don't fix -->
					<td <%=HtmlWriter.isVisible(oid, "rec-quantityDelivered")%> width="13%" height="18px" class="browseRow" align="center" nowrap>
						<input size=10 maxlength=15 name="ReceiptLine_qtyStep3Received" id="ReceiptLine_qtyStep3Received" value="<%=HiltonUtility.getFormattedQuantity(receiptLine.getQtyStep3Received(), oid)%>" style="text-align:right" onchange="checkBalance();">
					...
					<td <%=HtmlWriter.isVisible(oid, "rec-quantityRejected")%> width=13% class=browseRow nowrap align="center">
						<input type=text name="ReceiptLine_qtyStep3Rejected" size=10 value="<%=HiltonUtility.getFormattedQuantity(receiptLine.getQtyStep3Rejected(), oid)%>" style="text-align:right" onchange="setReceived(); checkRejected();">
					</td>
				<%	if (displayQtyAccepted) { %>
					<td <%=HtmlWriter.isVisible(oid, "rec-quantityAccepted")%> width=13% class=browseRow nowrap align="center">
						<input type=text name="ReceiptLine_qtyStep3Accepted" size=10 value="<%=HiltonUtility.getFormattedQuantity(receiptLine.getQtyStep3Accepted(), oid)%>" style="text-align:right" readonly>
					</td>
				<%	} else { %>
					<td <%=HtmlWriter.isVisible(oid, "rec-quantityAccepted")%> width=0% class=browseRow nowrap align="center">
						<input type=hidden name="ReceiptLine_qtyStep3Accepted" value="<%=HiltonUtility.getFormattedQuantity(receiptLine.getQtyStep3Accepted(), oid)%>">
					</td>
				<%	} %>
				...
				<tr>
					...
					<td colspan=5 class=browseRow valign=top align=right>
						...
						<tr <%=HtmlWriter.isVisible(oid, "rec-inspectionCode")%>>
							...
							<td class=browseRow>&nbsp;</td>
							<td height=16px>
								<input type=hidden name="ReceiptLine_rejectionCode" value="<%=receiptLine.getRejectionCode()%>" size=20 onchange="upperCase(this);">
							</td>
						</tr>
						...
						<tr id="ShelfLifeClauses">
							<td class=browseRow>&nbsp;</td>
							<td <%=HtmlWriter.isVisible(oid, "rec-shelfLifeClause")%> height=16px align="right" nowrap><%=DictionaryManager.getLabelsInstance(oid, language).getLabel(oid, "rec-shelfLifeClause", "Shelf Life Clause")%>:&nbsp;</td>
							<td class=browseRow><input type=text name="ReceiptLine_udf2Code" value="<%=receiptLine.getUdf2Code()%>" size=13 onchange="upperCase(this);"></td>
							<td class=browseRow>&nbsp;</td>
							<td class=browseRow>&nbsp;</td>
						</tr>
						<tr id="itemRejectionCode"></tr>
						<tr id="itemDispositionCode"></tr>
						...
						<table border=0 cellpadding="0" cellspacing="0" width="100%" height="100%">
							...
							<tr>
						        ...
						        <tsa:td field="rec-LN02">
						        	<tsa:input type="yesnoradio" title="" name="udf2Code" value="<%=s_udf2Code%>" disabled="disabled"/>
						        </tsa:td>
...
<!-- Hide script
	...
	function formatUnitPrice() {
		...
		var q = nformat(eval(nfilter(frm.ReceiptLine_qtyStep3Received)), qtyDecimals);
...
//			hideArea("invPropertyArea");
	...
	function receiveAllForItem() {
		...
		frm.ReceiptLine_qtyStep3Received.value = balance;
		frm.ReceiptLine_qtyStep3Rejected.value = nformat(0, qtyDecimals);
		checkBalance();

	}

	function checkBalance() {
		var receiptQty = nformat(eval(nfilter(frm.ReceiptLine_qtyStep3Received)), qtyDecimals);
		var rejectedQty = nformat(eval(nfilter(frm.ReceiptLine_qtyStep3Rejected)), qtyDecimals);
		...
		<% } else {
			if(receiptLine.getMarkTagRequired().equalsIgnoreCase("Y")){%>
				var quantity = frm.ReceiptLine_qtyStep2Accepted.value;
			<% } else if (receiptLine.getInspectionRequired().equalsIgnoreCase("Y")){%>
		...
		frm.ReceiptLine_qtyStep3Received.value = receiptQty;
		frm.ReceiptLine_qtyStep3Accepted.value = acceptedQty;
	...
	function checkProperty() {
		...
		var qtyReceived = frm.ReceiptLine_qtyStep3Accepted.value;
	...
	function setReceived() {
		var qtyReceived = nformat(eval(nfilter(frm.ReceiptLine_qtyStep3Received)), qtyDecimals);
		var qtyRejected = nformat(eval(nfilter(frm.ReceiptLine_qtyStep3Rejected)), qtyDecimals);
		var qtyReceivedValue = eval(nfilter(frm.ReceiptLine_qtyStep3Received));
		var qtyRejectedValue = eval(nfilter(frm.ReceiptLine_qtyStep3Rejected));

		frm.ReceiptLine_qtyStep3Rejected.value = qtyRejected;
		if (qtyRejectedValue > qtyReceivedValue) {
			...
			frm.ReceiptLine_qtyStep3Rejected.value = qtyReceived;
		}
		checkBalance();
	}
	...
	function returnRejected() {
		...
		var qtyReceived = eval(nfilter(frm.ReceiptLine_qtyStep3Received));
		...
		popupParameters = popupParameters + ";qtyRejected=" + frm.ReceiptLine_qtyStep3Rejected.value;
	...
	function retrieveLine(linenumber) {
		...
		doSubmit('/receipts/rec_item_step_3.jsp', 'ReceiptLineUpdate;ReceiptHeaderDataRetrieve;PoHeaderRetrieveById;ReceiptLineRetrieveByLineNumber');
	}
