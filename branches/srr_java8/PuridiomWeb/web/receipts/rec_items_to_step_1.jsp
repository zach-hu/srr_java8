...
<input type=hidden name="receiptStep" value="step_1">
...
<input type=hidden name="returnPage" value="/receipts/rec_items_to_step_1.jsp">
	...
	<td valign=top width=135px height=30px>
		...
		<tr>
			<td nowrap class=hdr12 valign=middle>
				<div style="margin-left:10px; margin-right:10px" class=hdr12><%=DictionaryManager.getLabelsInstance(oid, language).getLabel(oid, "rec-inspection", "Inspection")%></div>
			</td>
		</tr>
		</table>
	</td>
	...
	<td valign=top align=center>
		...
		<%	}
			for (int i = 0; i < i_linecount; i++)
			{
				...
				if(receiptLine.getStatus().compareTo(DocumentStatus.RCV_STEP_1) < 0 || receiptLine.getInspectionRequired().equalsIgnoreCase("N")){
				...
				if (qtyAccepted == null) qtyAccepted = new BigDecimal(0) ;
				if (qtyRejected == null) qtyRejected = new BigDecimal(0) ;
				inspected = qtyAccepted.add(qtyRejected) ;;
		...
		%>
				<tr <%=lineType %>>
					<td <%=HtmlWriter.isVisible(oid, "rec-lineNumber")%> width=8% height=18px class=browseRow nowrap>
						<% if(receiptLine.getStatus().compareTo(DocumentStatus.RCV_STEP_1) >= 0 && !receiptLine.getStatus().equals(DocumentStatus.CANCELLED)) {%>
					...
					<td <%=HtmlWriter.isVisible(oid, "rec-quantityReceived")%> width="10%" height="18px" class="browseRow" align="center" nowrap><%=HiltonUtility.getFormattedQuantity(qtyToInspect, oid)%></td>
					...
					<td <%=HtmlWriter.isVisible(oid, "rec-quantityInspected")%> width="10%" height="18px" class="browseRow" align="center" nowrap><%=HiltonUtility.getFormattedQuantity(inspected, oid)%></td>
...
<%	}%>
	...
	function recApproveStep1() {
			doSubmit('/receipts/rec_items_to_step_1.jsp', 'ReceiptUpdateNSteps;ReceiptLineRetrieveByHeader');
	}
