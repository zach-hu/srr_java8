...
<%
	...
	String attention = poHeader.getShipToContact();
	if (receiptMethod.equals("ReceiveByTransfer")) {
		attention = requisitionHeader.getShipAttn();
	}
%>
		...
		<tr>
			<td width=250px align=center valign=top>
				...
				<tr <%=HtmlWriter.isVisible(oid, "rec-shipToCode")%>>
					<td align=right nowrap width=50%><%=DictionaryManager.getLabelsInstance(oid, language).getLabel(oid, "rec-shipToCode", "Ship To", true)%>:&nbsp;</td>
					<td width=50%><input type=text name="ReceiptHeader_shipToCode" tabindex=1 size=15 maxlength=15 value="<%=receiptHeader.getShipToCode()%>" onchange="upperCase(this); getNewInfo('shipTo', this);" disabled></td>
				</tr>
			...
			<td width=250px align=center valign=top>
				...
				<tr <%=HtmlWriter.isVisible(oid, "rec-shp-attention")%>>
					...
					<td><input type=text name="PoHeader_shipToContact" tabindex=3 size=15 maxlength=40 value="<%=attention%>" onfocus="this.blur()" disabled></td>
				</tr>
				...
				<tsa:tr field="rec-udf10">
					<tsa:td field="rec-udf10" width="" align="right" noWrap="nowrap">
					<tsa:label labelName="rec-udf10" defaultString="UDF 10" noinstance="yes"/>:&nbsp;
					</tsa:td>
					<tsa:td field="rec-udf10">
					<tsa:input type="mintext" name="PoHeader_udf10Code" value="<%=poHeader.getUdf10Code()%>" onfocus="this.blur()" disabled="disabled"/>
					</tsa:td>
				</tsa:tr>
				<tsa:tr field="rec-priority">
					<tsa:td field="rec-priority" width="" align="right" noWrap="nowrap" >
					<tsa:label labelName="rec-priority" defaultString="Priority" noinstance="yes"/>:&nbsp;
					</tsa:td>
					<tsa:td field="rec-priority" >
					<tsa:input type="mintext" name="PoHeader_priorityCode" tabIndex="7" value="<%=poHeader.getPriorityCode()%>" onfocus="this.blur()" disabled="disabled"></tsa:input>
					</tsa:td>
				</tsa:tr>
