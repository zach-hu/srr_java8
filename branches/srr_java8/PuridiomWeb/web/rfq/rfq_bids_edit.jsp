...
<!--
	...
	if(IsMenu) {
		document.write("<SCR" + "IPT LANGUAGE='JavaScript1.2' SRC='<%=contextPath%>/scripts/rfq_bidArrays.js' TYPE='text/javascript'><\/SCR" + "IPT>");
		document.write("<SCR" + "IPT LANGUAGE='JavaScript1.2' SRC='<%=contextPath%>/scripts/hierMenusBids.js' TYPE='text/javascript'><\/SCR" + "IPT>");
	}
//-->
</SCRIPT>
...
<%	if (b_submit_access) {%>
		<tr>
			<td>
				...
				<tr>
					<td width=34% align=center><a href="javascript: window.top.hidePopWin();  void(0);"><img src="<%=contextPath%>/images/button_close.gif" class=button border=0 tabIndex=999></a></td>
					<td width=33% align=center><a href="javascript: submitBids(); void(0);"><img src="<%=contextPath%>/images/button_submit.gif" class=button border=0 tabIndex=110 alt="update bids"></a></td>
				</tr>
				</table>
			</td>
		</tr>
	...
	function submitBids() {
		...
		//	if ( checkRequired() ) {
				...
				if (frm.as_rfb_action.value == "UPDATE") {
					window.parent.setTimeout("b_submit=false;",10);
					window.parent.setTimeout("refreshBids();",3000);
...
<%	}%>
	...
	function doSubmitToPopup (url, handler, w, h) {
		window.parent.popupUrl = url;
		window.parent.popupHandler = handler;
		window.parent.popupUserId = frm.userId.value;
		window.parent.popupOrganizationId = frm.organizationId.value;
