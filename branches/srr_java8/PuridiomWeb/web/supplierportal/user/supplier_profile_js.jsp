...
<%	}%>
	...
	function checkImages() {
		var img = contextPath + "/images/delete.gif";
		for (var i = 0; i < 5; i++) {
	...
	function getRows () {
		...
		frm.as_rows.value = "1+++1+++1+++" + rowCnt + "+++1";
        }
	...
	function checkValues() {
		...
		validatePhone(frm.sup_fax_number, "<%=slb_fax%>");
		validatePhone(frm.con_phone_number, "<%=slb_phone%>");
		validatePhone(frm.con_fax_number, "<%=slb_contact_fax%>");
		validatePhone(frm.alt_phone_number, "<%=slb_alt_phone%>");
		validatePhone(frm.alt_fax_number, "<%=slb_alt_contact_fax%>");
		validateEmail(frm.con_email_addr, "<%=slb_email_addr%>");
		validateEmail(frm.alt_email_addr, "<%=slb_alt_email_addr%>");
...
<%	}
	if ( brq_address_line2 ) {%>
		ckRequired(frm.adr_address_line_2, "<%=slb_address_line2%>");
<%	}
	if ( brq_address_line3 ) {%>
		ckRequired(frm.adr_address_line_3, "<%=slb_address_line3%>");
<%	}
	if ( brq_address_line4 ) {%>
		ckRequired(frm.adr_address_line_4, "<%=slb_address_line4%>");
<%	}
	if ( brq_city ) {%>
		ckRequired(frm.adr_city, "<%=slb_city%>");
<%	}
	if ( brq_state ) {%>
		ckRequired(frm.adr_state, "<%=slb_state%>");
<%	}
	if ( brq_zip ) {%>
		ckRequired(frm.adr_postal_code, "<%=slb_zip%>");
<%	}
	if ( brq_country ) {%>
		ckRequired(frm.adr_country, "<%=slb_country%>");
<%	}
	if ( brq_fax ) {%>
		ckRequired(frm.sup_fax_number, "<%=slb_fax%>");
<%	}
/*****	SUPPLIER CONTACT REQUIRED FIELDS  ******/

	if ( brq_first_name ) {%>
		ckRequired(frm.con_first_name, "<%=slb_first_name%>");
<%	}
	if ( brq_middle_init ) {%>
		ckRequired(frm.con_middle_init, "<%=slb_middle_init%>");
<%	}
	if ( brq_last_name ) {%>
		ckRequired(frm.con_last_name, "<%=slb_last_name%>");
<%	}
	if ( brq_title ) {%>
		ckRequired(frm.con_contact_title, "<%=slb_title%>");
<%	}
	if ( brq_email_addr ) {%>
		ckRequired(frm.con_email_addr, "<%=slb_email_addr%>");
<%	}
	if ( brq_phone ) {%>
		ckRequired(frm.con_phone_number, "<%=slb_phone%>");
<%	}
	if ( brq_contact_fax ) {%>
		ckRequired(frm.con_fax_number, "<%=slb_contact_fax%>");
<%	}
/*****	ALTERNATE CONTACT REQUIRED FIELDS  ******/

	if ( brq_alt_first_name ) {%>
		ckRequired(frm.alt_first_name, "<%=slb_alt_first_name%>");
<%	}
	if ( brq_alt_middle_init ) {%>
		ckRequired(frm.alt_middle_init, "<%=slb_alt_middle_init%>");
<%	}
	if ( brq_alt_last_name ) {%>
		ckRequired(frm.alt_last_name, "<%=slb_alt_last_name%>");
<%	}
	if ( brq_alt_title ) {%>
		ckRequired(frm.alt_contact_title, "<%=slb_alt_title%>");
<%	}
	if ( brq_alt_email_addr ) {%>
		ckRequired(frm.alt_email_addr, "<%=slb_alt_email_addr%>");
<%	}
	if ( brq_alt_phone ) {%>
		ckRequired(frm.alt_phone_number, "<%=slb_alt_phone%>");
<%	}
	if ( brq_alt_contact_fax ) {%>
		ckRequired(frm.alt_fax_number, "<%=slb_alt_contact_fax%>");
<%	}
